...
> **対象読者**: [RAG手順書.md](./training_docs/RAG手順書.md) を完了し、シンプルRAGアプリを動かせる人
> **前提**: 既存の `lib/app.py`（Streamlit × LangChain × Chroma）が動く状態
...
- [ ] `RAG手順書.md` を完了している
- [ ] `~/Desktop/Self-RAG-Scratch/` プロジェクトに `lib/app.py` がある
- [ ] `streamlit run lib/app.py` が正常に動く
- [ ] `.venv` が有効化できる（`source .venv/bin/activate`）

### 1-2. 新しいファイルの置き場

既存の `lib/app.py` を**壊さずに**進めるため、別ファイル・別ディレクトリに書く。

```bash
cd ~/Desktop/Self-RAG-Scratch   # 既存プロジェクトに入る
...
Self-RAG-Scratch/
├── .venv/
├── .env.local              # GOOGLE_API_KEY などはここ
├── requirements.txt
├── chroma_db/              # 既存（lib/app.py で作成済み）
├── docs/
├── lib/
│   └── app.py              # 既存（Vanilla RAG、触らない）
...
>
> 📌 **重要**: 新規の `app_graph.py` と `rag/` は**プロジェクトルート直下**に置く。
> 既存の `lib/app.py` と並列ではなく、**ルートを基準にした構成**にすることで、
> `from rag.state import ...` の import や `chroma_db` の相対パス解決が素直に動く。
...
プロジェクトルート（`Self-RAG-Scratch/`）で実行：
...
`rag/nodes.py` を作成（プロジェクトルート直下の `rag/` 配下）。長いので**役割ごとに section を切って貼る**。
...
既存の `lib/app.py` と並列に、LangGraph 版の UI を作る。

`app_graph.py` をプロジェクトルート（`Self-RAG-Scratch/` 直下）に作成：
...
if not os.getenv("GOOGLE_API_KEY"):
    st.error(".env.local に GOOGLE_API_KEY を設定してください。")
    st.stop()
...
プロジェクトルート（`Self-RAG-Scratch/`）から起動する。
**`chroma_db` を相対パス `"chroma_db"` で参照しているため、必ずルートから起動すること。**

```bash
cd ~/Desktop/Self-RAG-Scratch
streamlit run app_graph.py
```

ブラウザで `http://localhost:8501` が自動で開く。
初回は `chroma_db/` が既に存在する前提（`lib/app.py` で作成済み）。

> 💡 既存版を比較したいときは別ターミナルで `streamlit run lib/app.py --server.port 8502` のように
> ポートを分けて並列起動すると差分が分かりやすい。
...
| `chroma_db` が空でエラー | 先に `streamlit run lib/app.py` でインデックスを作る |
| `ModuleNotFoundError: No module named 'rag'` | プロジェクトルート（`Self-RAG-Scratch/`）から起動していない。`cd` してから `streamlit run app_graph.py` |
| `chroma_db` が見つからない / 空の検索結果 | `lib/` から起動してしまっている可能性。ルートから起動しているか確認 |
...
```
~/Desktop/Self-RAG-Scratch/
├── .venv/
├── .env.local               # GOOGLE_API_KEY など
├── .gitignore
├── requirements.txt         # langgraph==0.2.60 が追加済み
├── app_graph.py             # ★新規（LangGraph 版エントリ、ルート直下）
├── rag/                     # ★新規（ルート直下）
│   ├── __init__.py
